...
export function BairroHistory({ city, uf, bairro }: Props) {
  ...
  const load = async () => {
    ...
    const { data: res, error: err } = await supabase.functions.invoke("overpass-history", {
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-3">
        <CardTitle className="flex items-center gap-2 text-base"><Clock className="h-4 w-4" /> Histórico OSM {bairro ? `· ${bairro}` : ""}</CardTitle>
      ...
      <CardContent>
        ...
        {!error && data.length === 0 && !loading && (
          <p className="text-sm text-muted-foreground">Histórico carregado sob demanda direto do Overpass — sem cache. Pode levar até 1 min.</p>
        )}
        {data.length > 0 && (
          <div className="h-64">
